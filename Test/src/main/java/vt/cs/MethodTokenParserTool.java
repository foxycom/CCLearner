...
public class MethodTokenParserTool extends Java8BaseListener {
    ...
    public TokenList getReservedWordTokenList(String methodBody) {
        ...
        int[] reservedType = new int[]{Java8Lexer.BOOLEAN, Java8Lexer.BYTE, Java8Lexer.CHAR, Java8Lexer.DOUBLE,
                Java8Lexer.FLOAT, Java8Lexer.INT, Java8Lexer.LONG, Java8Lexer.SHORT};
    ...
    public TokenList getOperatorTokenList(String methodBody) {
        ...
        for (Token token = lexer.nextToken(); token.getType() != Token.EOF; token = lexer.nextToken()) {
            if (token.getType() >= Java8Lexer.ASSIGN && token.getType() <= Java8Lexer.URSHIFT_ASSIGN) {
    ...
    public TokenList getMarkerTokenList(String methodBody) {
        ...
        for (Token token = lexer.nextToken(); token.getType() != Token.EOF; token = lexer.nextToken()) {
            if (token.getType() >= Java8Lexer.LPAREN && token.getType() <= Java8Lexer.DOT) {
